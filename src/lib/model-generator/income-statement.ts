...
import { ModelSheet, ModelLineItem, ModelProvenance, INCOME_STATEMENT_ITEMS } from './types'

const SEC_ARCHIVES_BASE = 'https://www.sec.gov/Archives/edgar/data'

/**
 * Build income statement model from XBRL data
 */
export function buildIncomeStatement(
  ...
  for (const itemDef of INCOME_STATEMENT_ITEMS) {
    ...
    if (periodValues.some((v) => v !== null)) {
      const item: ModelLineItem = {
        ...
        values: periodValues,
        unit: determineUnit(itemDef.standardLabel, values),
        provenance,
      }
  ...
  const sectionOrder = ['Revenue', 'Costs', 'Operating Expenses', 'Profit', 'Per Share']
...
function determineUnit(standardLabel: string, facts: NormalizedFact[]): string {
  if (standardLabel.includes('eps')) return 'USD_per_share'
  if (standardLabel.includes('shares')) return 'shares'

  const fact = facts[0]
  if (!fact) return 'USD'

  if (fact.unit === 'USD') {
    // Check magnitude to determine display unit
